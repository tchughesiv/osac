...
- `create_compute_instance`: create a VM from a published, policy-controlled
  offering, with optional instance-type, boot-disk, and network-attachment
  overrides that use discovered IDs.
...
- A self-contained Kind `dev-full` demo target builds the current checkout's
  image, loads it directly into Kind, and seeds a virtual-machine offering from
  AAP-published template data. Validate its request and reconciliation flow
  locally; use a Linux host with nested KVM support to demonstrate a running
  guest VM.
...
- The demo installer supplies its platform and tenant prerequisites: KubeVirt,
  CDI, storage, AWX publication, a hub, and a ready default VirtualNetwork,
  Subnet, and SecurityGroup. A ComputeInstance does not invent them
  dynamically.
