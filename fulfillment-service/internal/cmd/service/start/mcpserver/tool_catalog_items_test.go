...
var _ = Describe("handleDescribeCatalogItem", func() {
	It("Maps the resolved item's field policies and forwards the caller's token", func() {
		defaultSSHKey := "ssh-ed25519 AAAA"
		region, err := anypb.New(wrapperspb.String("us-east-1"))
		Expect(err).ToNot(HaveOccurred())
						...
						publicv1.ClusterCatalogItem_builder{
							...
							Fields: publicv1.ClusterCatalogItemFields_builder{
								SshPublicKey: publicv1.StringFieldPolicy_builder{
									Editable: publicv1.EditableStringField_builder{
										DefaultValue: proto.String(defaultSSHKey),
									}.Build(),
								}.Build(),
							}.Build(),
							TemplateParameters: map[string]*publicv1.TemplateParameterPolicy{
								"region": publicv1.TemplateParameterPolicy_builder{
									Editable: publicv1.EditableTemplateParameter_builder{
										DefaultValue: region,
									}.Build(),
								}.Build(),
							},
		...
		Expect(output.Fields).To(Equal(map[string]any{
			"sshPublicKey": map[string]any{
				"editable": map[string]any{"defaultValue": defaultSSHKey},
			},
		}))
		Expect(output.TemplateParameters).To(Equal(map[string]any{
