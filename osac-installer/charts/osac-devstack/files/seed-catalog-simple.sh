...
create_or_update_catalog_item() {
  local create_data="$1"
  local fields="$2"
  local output
  local catalog_item_id
  local update_data

  if output="$(grpc_call "osac.private.v1.ComputeInstanceCatalogItems/Create" "${create_data}" 2>&1)"; then
    log "  created catalog-item: linux-vm"
    return
  fi
  if [[ "${output}" != *"AlreadyExists"* && "${output}" != *"already exists"* ]]; then
    printf 'ERROR: failed to create catalog-item linux-vm:\n%s\n' "${output}" >&2
    return 1
  fi

  if ! output="$(grpc_call "osac.private.v1.ComputeInstanceCatalogItems/List" '{}' 2>&1)"; then
    printf 'ERROR: failed to list catalog items while reconciling linux-vm:\n%s\n' "${output}" >&2
    return 1
  fi
  catalog_item_id="$(find_named_item_id "linux-vm" <<<"${output}")"
  if [[ -z "${catalog_item_id}" ]]; then
    printf 'ERROR: catalog-item linux-vm already exists but could not be resolved\n' >&2
    return 1
  fi

  update_data="{\"object\":{\"id\":\"${catalog_item_id}\",\"fields\":${fields}},\"updateMask\":{\"paths\":[\"fields\"]}}"
  if output="$(grpc_call "osac.private.v1.ComputeInstanceCatalogItems/Update" "${update_data}" 2>&1)"; then
    log "  updated catalog-item: linux-vm"
    return
  fi

  printf 'ERROR: failed to update catalog-item linux-vm:\n%s\n' "${output}" >&2
  return 1
}
...
# The MCP demo can select from these existing, tenant-visible resources. The
# fulfillment service remains the policy authority: it accepts an override only
# because this catalog item marks the corresponding field editable.
catalog_item_fields='{"instanceType":{"editable":{}},"bootDisk":{"sizeGib":{"editable":{}},"storageTier":{"editable":{}}},"networkAttachments":{"editable":{}}}'
create_or_update_catalog_item \
  "{\"object\":{\"metadata\":{\"name\":\"linux-vm\"},\"title\":\"Linux Virtual Machine\",\"description\":\"Fedora-based VM with KVM acceleration\",\"template\":{\"id\":\"osac.templates.ocp_virt_vm\"},\"published\":true,\"fields\":${catalog_item_fields}}}" \
  "${catalog_item_fields}"
